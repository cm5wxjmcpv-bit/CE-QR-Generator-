...
function ceShiftToken() {
  if (window.CE_ADMIN_TOKEN) return window.CE_ADMIN_TOKEN;
  if (typeof ADMIN_TOKEN !== "undefined" && ADMIN_TOKEN) return ADMIN_TOKEN;
  return "";
}
...
async function addCeRecordToShift(record, shiftId, statusElement) {
  ...
  const token = ceShiftToken();
  if (!token) throw new Error("Admin token not found. Refresh the admin page and log in again.");
  ...
  shifts.forEach(shift => {
    ...
    shift.members.forEach(existing => {
      ...
      if (sameProvider && shift.id === target.id) alreadyInTarget = true;
      if (!sameProvider) keptMembers.push(existing);
    });
    shift.members = keptMembers;
  });

  if (!alreadyInTarget) target.members.push(member);
  ...
  if (status) {
    status.textContent = alreadyInTarget ? `Already in ${target.label}.` : `Added to ${target.label}.`;
  }
}
